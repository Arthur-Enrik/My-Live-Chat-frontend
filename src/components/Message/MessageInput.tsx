...
function MessageInput({sendMessage}: Props) {
    ...
    <form onSubmit={submitHandler} className='flex h-fit w-full gap-1 bg-zinc-800 text-white rounded-md mt-auto'>
        <input value={message} onChange={e => {setMessage(e.target.value)}} type="text" className='w-full h-10 rounded-md p-2 outline-none' />
