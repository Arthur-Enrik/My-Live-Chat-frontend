...
function ContactsNavbar({chats, setActiveChat}: Props) {
    ...
    return(
        <nav className="w-1/4 flex flex-col text-white bg-zinc-800 rounded-md relative">
            ...
            <ul className="w-full flex flex-col overflow-y-auto divide-gray-100 divide-y">
                {chats ? chats.map(item => <Contact chatName={item} setActiveChat={setActiveChat} key={item} />) : <span className="w-full text-center text-white p-3">Você ainda não possui nenhum contato</span> }
            </ul>
